function (user, context, callback) {
   ...
   */
  const SLASH_GRAPHQL_ENDPOINT = "${REACT_APP_GRAPHQL_ENDPOINT}";
  const SLASH_AUTH_NAMESPACE = "${REACT_APP_AUTH_NAMESPACE}";
  const CLIENT_ID = "${REACT_APP_AUTH0_CLIENT_ID}";

  /**
   * NOTE: If you are using a single auth0 account with multiple applications,
   * you probably want to apply a check of the context.clientID to match this
   * application before running this rule. The following line will skip
   * this rule if the client IDs don't match.
   */
